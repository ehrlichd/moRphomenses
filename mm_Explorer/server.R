...
server <- function(input, output) {
  ...
  observeEvent(input$assign_id,{
    ...
    rv$n_inds <-  nrow(count(rv$mm_dataset, !!sym(eval(input$assign_id))))
    })
  ...
  output$dyn_pick1 <- renderUI({
    ...
    selectInput("ind_pick1",
                label = "Individual to highlight",
                choices = ind_list
                )
  ...
  output$pc_scatter <- renderPlot(
    # height = pc_shape_width, width = pc_shape_width,
    {
      ...
      plot(rv_pca_select()[,c(1,2)],
           main = "", xlab = "", ylab = "",
           as= T, bty = "n", xaxt = "n", yaxt = "n",
           type = "n", xlim = x_r, ylim = x_r,
           pch = 21, bg = "dark grey")
    ...
    ##### text(xax, rep(0, 5), labels = round(xax),cex = 1, col = "blue", pos = 1)
      ...
      ## add target
      abline(h = selected_coords$y,
             col = "green")
      abline(v = selected_coords$x,
             col = "green")
      points(selected_coords$x,
             selected_coords$y,
             pch = 12, col = "green", cex = 2)
  ...
  output$pc_x_max <- renderPlot(
    # height = pc_shape_height, width = pc_shape_width,
    {
      ...
      #   return(NULL)
      # }
    mm_PlotArray(rv_pca_full()$Shapes[[input$x_pc]]$max, lbl = "")
  })
  ...
  output$phenotypes <- renderPlot(
    height = reactive(200*brush_ind$max_k),
    width = 600,{
